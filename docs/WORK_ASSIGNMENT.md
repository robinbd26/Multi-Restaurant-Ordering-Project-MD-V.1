...
| ~~WS-4.5~~ **DONE (24 Sep 2026)** — the two models are now one: coverage AND price come from a drawn `BranchDeliveryArea.shape`, `BranchDeliveryZone` is gone, and the area is resolved from the delivery POINT rather than chosen by the client | Location | High | M | Spatial Data Engineer | `lib/services/delivery.ts:23-51`, `lib/services/delivery-areas.ts:347-356`, `prisma/schema.prisma:564-615` |
...
> **DELIVERED, then re-platformed (24 Sep 2026).** The MapPicker, the server-side geocoding
> helper and the coordinate-provenance work below all shipped. They no longer use Google:
> maps are Leaflet + OpenStreetMap (no key), and geocoding is Barikoi behind /api/geo/*
> (BARIKOI_API_KEY, server-only). Build against docs/map-coverage-plan.md, not the Google

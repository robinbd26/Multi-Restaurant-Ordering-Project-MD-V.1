...
export async function generateMetadata(): Promise<Metadata> {
  const { t } = await getT();
  return { title: t("rider.supportTitle") };
}

/** /rider/support — support contact + issue categories (future-ready). */
export default async function RiderSupportPage() {
  const { t } = await getT();
  await requireRole("rider");

  const categories = [
    { icon: "bike", label: t("rider.issueDelivery") },
  ...
  return (
    <>
      <PageHeader title={t("rider.supportTitle")} subtitle={t("rider.supportSub")} />
